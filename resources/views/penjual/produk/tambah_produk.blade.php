        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 max-w-5xl">
            ...
            <form action="{{ route('produk.store', ['token' => request('token')]) }}"
                ...
                <div>
                    ...
                    <button type="button" onclick="addVariant()" class="mt-3 px-4 py-2 bg-blue-500 text-white rounded-lg text-sm">
                        + Tambah Variant
                    </button>
                </div>
                ...
                <div>
                    ...
                    <input type="file" name="foto_produk" accept="image/*" id="foto_produk_input" class="block">
                    <div class="mt-4">
                        <img id="previewImage" class="hidden w-40 h-40 object-cover rounded-xl border border-slate-200">
                    </div>
                </div>

                <div class="flex justify-end gap-3 pt-6 border-t border-slate-200">
                    <a href="{{ route('produk.list_produk', ['token' => request('token')]) }}"
                       class="px-5 py-2.5 rounded-xl bg-slate-200 text-slate-700">Batal</a>

                    <button type="submit"
                            class="px-6 py-2.5 rounded-xl bg-green-600 text-white">
                        Simpan Menu
                    </button>
                </div>
            </form>
...
function addVariant() {
    ...
    container.insertAdjacentHTML('beforeend', `
        <div class="grid grid-cols-12 gap-3 variant-item">
            <input type="text" name="nama_variant[]" placeholder="Nama Variant" class="col-span-6 w-full h-10 px-3 border rounded-lg">
            <input type="number" name="harga_variant[]" placeholder="Harga Tambahan" class="col-span-5 w-full h-10 px-3 border rounded-lg">
            <button type="button" onclick="removeVariant(this)" class="col-span-1 h-10 rounded-lg bg-red-500 text-white">X</button>
        </div>
    `);
}
